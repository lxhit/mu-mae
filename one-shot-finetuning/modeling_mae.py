...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x,acc_phone_data,acc_watch_data,gyro_data,orientation_data):
        ...
        x_combine = torch.cat((x, phone_x, watch_x, gyro_x, ori_x), dim=1)

        x_combine = self.pos_drop(x_combine)
        ...
        if self.fc_norm is not None:
            return self.fc_norm(x_combine.mean(1))
        else:
            return x_combine[:, 0]
    def get_patch_sensor_data(self, sensor_data):
        sensor_patch_data = rearrange(sensor_data, 'b (t p) l s -> b p (t l s)',
                                      p=16)  # (5,128,1,3)--(32,16,8*1*3*1)
        return sensor_patch_data

    def forward(self, x,acc_phone_data,acc_watch_data,gyro_data,orientation_data):
        # print("acc_phone_data shape in modeling mae: " + str(acc_phone_data.shape))
        acc_phone_data = self.get_patch_sensor_data(acc_phone_data)
        acc_watch_data = self.get_patch_sensor_data(acc_watch_data)
        gyro_data = self.get_patch_sensor_data(gyro_data)
        orientation_data = self.get_patch_sensor_data(orientation_data)
        x = self.forward_features(x,acc_phone_data,acc_watch_data,gyro_data,orientation_data)
        # print("x return shape in modeling mae: " + str(x.shape))
        return x
